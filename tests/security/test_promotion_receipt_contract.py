...
other, which is what keeps the five proofs from being decorative.
...
class HonestHeadTests(unittest.TestCase):
    ...
    def test_a_two_workload_receipt_still_fits_the_contract_ceiling(self):
        ...
        widest = MODULE.approve_receipt(self.head, self.head, surface, [statement, statement], "2026-09-04", READY.body_digest(self.fixture.body))
...
class PostingTests(unittest.TestCase):
    ...
    def test_a_subject_withdrawn_during_token_acquisition_is_not_posted(self):
        # Design reset (PR #312 round 4): the whole subject — head, open,
        # Draft, `requires-review` — is re-read after the token is minted,
        # immediately before the post. Closed, readied or disarmed meanwhile,
        # the head unchanged: nothing is posted.
        for change in ("close", "ready", "disarm"):
            with self.subTest(change=change):
                ...
                fixture.step()
                self.assertEqual(fixture.posted(), [], change)
                self.assertTrue(any("during token acquisition; nothing was posted" in line for line in fixture.log_lines), change)

    def test_a_head_outside_the_review_subject_is_not_judged(self):
        # Design reset (PR #312 round 4): a pull request that is not open, not
        # Draft, or no longer wears the label the promoter armed is a subject
        # nobody asked about. No proof runs, nothing is posted, and — above
        # all — the label is never put back by the tool.
        for change in ("closed", "ready", "disarm"):
            with self.subTest(change=change):
                fixture = ProofFixture(self)
                fixture.cut()
                if change == "closed":
                ...
                fixture.step()
                self.assertEqual(fixture.posted(), [], change)
                self.assertEqual([call for call in fixture.calls if call["argv"][0] == HELPER], [], "no token was minted")
                self.assertFalse(any(line.startswith("PROOF identity") for line in fixture.log_lines), "no proof ran")
                self.assertTrue(any(line.startswith("PROOF subject") and "skipped:" in line for line in fixture.log_lines), change)
                self.assertEqual([call for call in fixture.writes() if call["write"].endswith("/issues/300/labels")], [], "the tool never arms the label")

    def test_a_retirement_outrun_by_a_moved_head_is_left_for_a_person(self):
        # Design reset (PR #312 round 4): the label DELETE landed by number,
        # the head moved, and the replacement head is left without review
        # attention. The tool never re-arms: the next tick finds a subject
        # nobody asked about and says so.
        self.fixture.move_head_after = "label"
        self.fixture.step()
        self.assertEqual(len(self.fixture.posted()), 1)
        self.assertFalse(self.fixture.review_armed, "the retirement outran the moved head")
        self.fixture.step()
        self.assertFalse(self.fixture.review_armed, "the tool did not re-arm the replacement head")
        self.assertEqual([call for call in self.fixture.writes() if call["write"].endswith("/issues/300/labels")], [])
        self.assertTrue(any("requires-review is not armed; nobody asked for a receipt" in line for line in self.fixture.log_lines))
        self.assertEqual(len(self.fixture.posted()), 1, "a head nobody asked about is never judged")

    def test_the_approve_receipt_binds_the_body_it_audited(self):
        # Design reset (PR #312 round 4): the body is same-head-mutable, so the
        # receipt states its digest and the Ready rule recomputes it live.
        verdict, receipt = self.fixture.prove()
        self.assertEqual(verdict, "APPROVE")
        digest = READY.body_digest(self.fixture.body)
        self.assertIn(f"\n{READY.BODY_BINDING}{digest}\n", receipt)
        self.assertEqual(digest, READY.body_digest(self.fixture.body.replace("\n", "\r\n") + "\n\n"), "the digest survives newline normalisation")
        labels = list(MODULE.PR_LABELS)
        comments = [app_comment(self.head, receipt)]
        lanes, _, blockers = READY.ready_decision(self.head, labels, comments, self.fixture.checks, 0, "open", "main", "main", body=self.fixture.body)
        self.assertEqual(blockers, [])
        self.assertEqual(lanes, ["promoter"])
        lanes, _, blockers = READY.ready_decision(self.head, labels, comments, self.fixture.checks, 0, "open", "main", "main", body=self.fixture.body + "\nOne more sentence.")
        self.assertEqual(lanes, [], "an edited body leaves no approving lane")
    ...
    def test_each_failure_receipt_states_what_its_proof_compared(self):
        # PR #312 round 4, finding 2: the durable statement of what a failed
        # proof COMPARED is bound per proof, not only the proof's name.
        phrases = {
            "Confinement": ("changed-path set against `main`", "paths a re-derived cut writes"),
            "Re-derivation": ("compared entry by entry", "promotion surface a re-derived cut writes"),
            "Claim audit": ("re-composed from the re-derived records", "compared byte for byte"),
            "Identity": ("SSH signature", "registered signing keys"),
        }
        head, base = "a" * 40, "b" * 40
        for proof, own in phrases.items():
            with self.subTest(proof=proof):
                receipt = MODULE.request_changes_receipt(head, base, proof, "detail")
                self.assertIn(f"1. {proof} failed: detail", receipt)
                for phrase in own:
                    self.assertIn(phrase, receipt)
                for other, theirs in phrases.items():
                    if other != proof:
                        for phrase in theirs:
                            self.assertNotIn(phrase, receipt, (proof, other))
...
class ContractTextTests(unittest.TestCase):
    def test_agents_states_the_earned_receipt_and_keeps_the_ready_rule_verbatim(self):
        ...
        self.assertIn("open, Draft and wearing `requires-review`", MODULE.__doc__)
        self.assertNotIn("nothing about the pull request's state", MODULE.__doc__)
        self.assertNotIn("re-armed at", source)
        self.assertNotIn("def rearm_review_attention", source)
        self.assertIn("never re-arms", MODULE.__doc__)
        self.assertIn("compared by mode, type and object id", source)
        self.assertIn("by mode,\n   type and Git object id", RUNBOOK.read_text(encoding="utf-8"))
        self.assertIn("`BODY-SHA256:` line", RUNBOOK.read_text(encoding="utf-8"))
        self.assertIn("it never re-arms that\n  label", text)
        self.assertIn("binds\n  the digest of the body it audited", text)
