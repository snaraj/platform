...
[`site_sync_branch_flip.py`](./site_sync_branch_flip.py) makes every judgment of the issue-275 decoupling ceremony (`docs/runbooks/site-sync-branch-flip.md`) fail-closed and testable: the full-document no-bypass ruleset receipt with every rule parameter compared, the closed GitRepository shape (reviewed URL/interval/ignore, exactly the two sparse-checkout site directories, structural absence of every credential/submodule/impersonation field), selector-identity-bound quiescence by owner-UID lineage over unfiltered status-complete inventories, atomically written private prestate captures for both the source and the selector, UID- and field-bound compare-and-swap suspend/flip/rollback/resume patches that preserve the selector's pre-ceremony state, exact poststate and rollback verification over the full captured spec, the runbook's command blocks pinned byte-for-byte against the tool's own `CEREMONY_BLOCKS` canon, and the outside-in `verify-live` probe that fetches the live page's sole fingerprinted asset and requires its exact served bytes inside the committed chart's hash-verified image layers.
...
[`ci/run_python_tests.py`](./ci/run_python_tests.py) runs the complete discovered unittest inventory in at most four processes, requires exact execution counts and successful outcomes from every worker, and retains coverage measurement in workers and test subprocesses.
...
forge by the coordinator. Meanwhile
...
existing path.
Those three primitives remain non-authoritative legacy surfaces; no active site
