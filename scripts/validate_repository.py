...
# Every namespace that owns a direct Flux reconciliation root under
# `kubernetes/websites/`. The two websites, plus the `obsidian` workload the
# owner admitted on 2026-09-07 (issue #348). This drives the RBAC topology, the
# default-deny ownership and the source artifact boundary — the reconciliation
# graph, which every workload joins the moment its root exists.
#
...
# chart identity). `obsidian` is reconciled but not activated: its chart
...
# `obsidian` runs one stateful single-writer server whose per-Pod limit is ten
...
REVIEWED_NAMESPACE_CAPACITY = {
    ...
    "obsidian": (
        Path("docs/audits/2026-09-07-obsidian-capacity-evidence.md"),
        {
            "pods": "2",
            "requests.cpu": "200m",
...
FLUX_ACCESS_CONTRACT_SHA256 = (
    "f396a9844e7c57fcd9851e0d281ab46efd2bb474f3038042519a4f4e1fd7a544"
)
...
def check_media(root):
    ...
    source_required = (
        ...
        for namespace in DIRECT_WORKLOAD_NAMESPACES
        for line in (
            "!/kubernetes/websites/{}/".format(namespace),
            "!/kubernetes/websites/{}/**".format(namespace),
        )
    )
    if not sourceignore.is_file():
        ...
        if source_lines != source_required:
            errors.append(
                "Flux source artifact allowlist is not the exact reconciled-workload boundary"
            )
    ...
    expected_role_rules = {
        ...
        ("flux-system", "flux-controller-impersonation"): (
            expected_rule(
                ...
                tuple(name + "-reconciler" for name in DIRECT_WORKLOAD_NAMESPACES),
            ),
        ),
    }
    for site in DIRECT_WORKLOAD_NAMESPACES:
