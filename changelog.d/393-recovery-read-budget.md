...
- Freeze the thirteenth historical-recovery edge, `v0.1.93`, so the merge that derives those bounds stays publishable after main moves past it, and pin the executor it froze on all eight edges that executor already published (`v0.1.82`-`v0.1.89`), each value read from that edge's own immutable identity asset, so freezing it does not refuse them as foreign.
