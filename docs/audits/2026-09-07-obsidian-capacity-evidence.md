...
obsync workload into its own namespace, named `obsidian`). The quota is bound
...
`docs/platform-onboarding.md` item 8) and restated in
`kubernetes/websites/obsidian/release.yaml` so the quota and the Pod it pays
for are one reviewable pair inside this repository:
...
stateless Go web server at 25m/32Mi requests and 200m/128Mi limits. obsidian
