# SSH-only administrative ingress procedures
...
CNI traffic alone. Captures use `nft -a -j list ruleset` so the verifier can
require the exact current envelope and metainfo, uint64 table/chain/rule
handles, distinct owned-rule handles, literal table/chain/rule key sets,
integer priority `-10`, and anonymous `{packets,bytes}` uint64 counters. It
rejects duplicate JSON members, non-finite numeric encodings, excessive JSON
nesting, every table flag (including `dormant`), sets, maps, jumps, wildcard
matches, single-family variants, unknown grammar, extra objects, missing
counters, and same-name decoys.
Diagnostics are fixed tokens and never contain an interface, address, peer,
ruleset, private count, or a hash of the private contract.
...
The selected recovery snapshot must match the loaded journal's source revision,
manifest, custody directory, and custody-receipt hash before it may use any
journal-selected custody byte or host/API operation. A mismatch or any other
pre-mutation validation failure preserves the prior journal and receipt set
byte-for-byte. Recovery arms `recovery-required` publication only after a
durable `rollback-intent`; that failure receipt conservatively marks current
boot, cluster, and private-contract verification as `unverified`.
...
splits; cross-bundle recovery refusal; closed-journal preservation;
stale/tampered bindings; source symlink/hard-link/race attacks; exact rollback;
and zero temporary residue. It does not prove the Pi kernel,
